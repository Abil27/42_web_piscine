/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   sing_it!.js                                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ahoussei <ahoussei@student.42.us.org>      +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/12/28 13:58:11 by ahoussei          #+#    #+#             */
/*   Updated: 2018/12/29 13:57:02 by ahoussei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

args = process.argv.slice(2).join('');

  if(args === "mais pourquoi cette demo ?"){
    console.log("Tout simplement pour qu'en feuilletant le sujet on ne s'apercoive pas de la nature de l'exo");
  } else if (args === "mais pourquoi cette chanson ?"){
    console.log("Parce que Kwame a des enfants");
  } else if (args === "vraiment ?"){
    console.log("Nan c'est parce que c'est le premier avril")
  }
    

  //not completed have to do the last statement
